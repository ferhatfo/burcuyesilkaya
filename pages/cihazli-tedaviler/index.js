...
export async function getStaticProps() {
  ...
  const cihazliTedaviler = treatments.filter(category => category.category === 'Cihazlı Tedaviler');
  
  return {
    props: {
      cihazliTedaviler: cihazliTedaviler.length > 0 ? cihazliTedaviler : [],
    },
  };
}

export default function CihazliTedavilerPage({ cihazliTedaviler }) {
  return (
    <>
      <Head>
        <title>Cihazlı Tedavilerimiz - Uzman Dr. Burcu Yeşilkaya</title>
        <meta name="description" content="Uzman Dr. Burcu Yeşilkaya olarak sunduğumuz modern cihazlı tedavi yöntemlerini inceleyin." />
      </Head>

      <MultiPageHeader
        subtitle="Cihazlı Tedavilerimiz"
        title="Uzman Dr. Burcu Yeşilkaya"
        isImage={false}
      />
       
      <div className="container mx-auto px-4 py-8 md:py-12 max-w-7xl">
        {cihazliTedaviler.map((category, categoryIndex) => (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-6 py-8 bg-gray-100 rounded-xl rounded-tl-none rounded-tr-none flex flex-col flex-1 min-h-0">
                      ...
                      <Link href={`/cihazli-tedaviler/${slug}`} className="flex-shrink-0">
                        <Button
                          text="Detayları Gör"
